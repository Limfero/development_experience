...
INTERFACE
  ...
  PROCEDURE ReadMonth(VAR FIn: TEXT; VAR Mo: Month); {чтение мес€ца из файла}
  PROCEDURE WriteMonth(VAR FOut: TEXT; VAR Mo: Month); {запись мес€ца в файл}
IMPLEMENTATION
  PROCEDURE ReadMonth(VAR FIn: TEXT; VAR Mo: Month); {„итает значение из FIn и записывает в Mo}
  VAR
    Ch1, Ch2, Ch3 : CHAR;
  BEGIN{ReadMonth}
    ...
    IF (Ch1 = 'D') AND (Ch2 = 'E') AND (Ch3 = 'C') THEN Mo := DEC ELSE
      Mo := NoMonth
  END; {ReadMonth}
  PROCEDURE WriteMonth(VAR FOut: TEXT; VAR Mo: Month); { „итает значение из Mo и записывает в FOut}
  VAR
    Ch1, Ch2, Ch3 : CHAR;
  BEGIN {WriteMonth}
